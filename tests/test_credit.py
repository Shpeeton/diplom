...
class TestCredit:
    ...
    @allure.story("Кредитная оплата")
    @allure.title("Успешная оплата в кредит")
    def test_success_credit(self, driver, db_and_url):
        db, app_url = db_and_url
        page = MainPage(driver, app_url).open().credit()
    ...
    @allure.story("Кредитная оплата")
    @allure.title("Отказ в кредите")
    def test_declined_credit(self, driver, db_and_url):
        db, app_url = db_and_url
        page = MainPage(driver, app_url).open().credit()
    ...
    @allure.story("Кредитная оплата")
    @allure.title("Невалидный год (00)")
    def test_invalid_year_00(self, driver, db_and_url):
        db, app_url = db_and_url
        page = MainPage(driver, app_url).open().credit()
        page.fill_card(CardData.APPROVED_CARD)
        page.fill_month(CardData.VALID_MONTH)
        page.fill_year(CardData.INVALID_YEAR_00)
        page.fill_owner(CardData.VALID_OWNER)
        page.fill_cvc(CardData.VALID_CVC)
        page.submit()
